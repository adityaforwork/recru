...
export default function BulkUploadModal({ isOpen, onClose, onUploadSuccess }) {
  ...
  const closeConfirm = () => setConfirm({ open: false, title: "" });
  ...
  const handleConfirmUpload = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setConfirm({ open: true, title: result.message || `${previewData.length} candidates imported` });
      } else {
  ...
  return (
    <>
      {isOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
